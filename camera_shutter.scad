...
module cots_dc5v_camera_shutter(show=true, drill=false){
     //
     // Uses "DC 5V Digital Camera Shutter Micro Rotating electromagnet Control Switch"
     // from ebay:
     // https://www.ebay.com/itm/124456754185?hash=item1cfa335c09:g:8tIAAOSwq4Nh7qDp
     //
     // oriented with shutter in yz plane, optical axis along x, magnet housing pointing
     // to +x and sitting above shutter.
     //
     ...
     cs_bot_strut_dz = 3.0;
     cs_bot_strut_hole_zpos = (cs_bot_strut_dz - cs_bot_strut_hole_dia)/2 + cs_bot_strut_hole_dia/2;
     cs_bot_strut_hole_xpos = -cs_dx/2 -cs_bot_strut_dx + cs_bot_strut_hole_zpos;
	  ...
	  screw_head_yoff = cs_dy/2 + 2;
	       color("red"){
		    translate([0, 0, cs_aperture_zpos])
			 rotate([0, 90, 0])
			 translate([0, 0, -10]) cylinder(d=cs_aperture_dia, h=20);		// main aperture for optical beam path
		    translate([cs_bot_strut_hole_xpos, 0, cs_bot_strut_hole_zpos]){
			 rotate([90, 0, 0]){
			      translate([0, 0, -10]) cylinder(d=cs_bot_strut_hole_dia, h=20);		// bottom strut hole
			      translate([0, 0, screw_head_yoff]) cylinder(d=screw_clear_head_dia_1_7mm, h=20);		// screw head
			 }
		    }
		    translate([up_strut_hole_xpos, 0, up_strut_hole_zpos]){
			 rotate([90, 0, 0]){
			      translate([0, 0, -10]) cylinder(d=up_strut_hole_dia, h=20);		// up strut hole
			      translate([0, 0, screw_head_yoff]) cylinder(d=screw_clear_head_dia_1_7mm, h=20);		// screw head
			 }
		    }
	       }
     ...
     translate([0, 0, -cs_aperture_zpos]){
	  difference(){
	       union(){
		    ...
		    color("silver")
			 translate([-cut_dx/2, -cs_dy/2, 0])
			 cube([cut_dx, cs_dy, cs_dz]);
		    color("gray")
			 translate([-cut_dx/2, -cs_bot_stub_dy + cs_dy/2, -cs_bot_stub_dz + 0.01])	// stub on bottom right
			 cube([cut_dx, cs_bot_stub_dy, cs_bot_stub_dz]);
		    color("black")
			 translate([-cut_bot_strut_dx - cs_dx/2, -cs_dy/2, 0])			// strut bottom left back
			 cube([cut_bot_strut_dx, cs_bot_strut_dy, cs_bot_strut_dz]);
		    color("black")
			 translate([cs_dx/2, -cs_dy/2, up_strut_zpos])			// strut upper left front
			 cube([cut_up_strut_dx, up_strut_dy, up_strut_dz]);
		    color("black")
			 translate([-cs_dx/2 + bot_pin_xoff, -cs_dy/2-bot_pin_dy, bot_pin_zoff])	// bottom pin
			 cube([bot_pin_dx, bot_pin_dy, bot_pin_dz]);
		    color("green")
			 translate([-cs_dx/2, 0, cs_magnet_zoff])	// magnet cylinder
			 rotate([0, 90, 0])
	  ...
	  if (drill){
	       color("pink"){
		    ...
		    translate([-cut_bot_strut_dx - cs_dx/2, -cs_dy/2, 0])			// slot for bot strut
			 cube([cut_bot_strut_dx, cs_bot_strut_dy, cs_bot_strut_dz + 10]);
		    translate([-cs_dx/2 + bot_pin_xoff, -cs_dy/2-bot_pin_dy + 0.01, 0])	// bottom pin
			 cube([bot_pin_dx, bot_pin_dy, 16]);
		    translate([-cs_dx/2, -bot_mag_cut_dy/2, bot_mag_cut_zoff])	// cutout to access bottom magnet pins
			 cube([bot_mag_cut_dx, bot_mag_cut_dy, bot_mag_cut_dz]);
		    translate([-bot_front_cut_dx, -bot_mag_cut_dy/2, bot_mag_cut_zoff])	// cutout for bottom front
...
module camera_shutter(dz=12.7, show_mount=true, show_electronics=true, include_electronics_mount=true){
     //
     ...
     difference(){
	  union(){
	       color("orange"){
		    hull(){
			 ...
			 translate([-cs_housing_dx/2 + cs_housing_xoff, -cs_housing_dy/2, cs_housing_zoff])
			      cube([cs_housing_dx, cs_housing_dy, cs_housing_dz]);	// housing for DC solenoid camera shutter
		    }
	       }
	       if (include_electronics_mount){
		    color("blue"){
			 ...
			 for(m=[0, 1]){
			      mirror([0, m, 0]){
				   hull(){
					translate([-elec_base_dx/2 + base_xoff, -elec_base_dy + base_dy/2, -dz + base_dz])
					     cube([elec_base_dx, elec_base_dy, 0.1]);		// cube for base
					translate([elec_standoff_xoff, elec_standoff_yoff, elec_standoff_zoff])
			 ...
			 for(m=[0, 1]){
			      mirror([0, m, 0]){
				   translate([elec_standoff_xoff, elec_standoff_yoff, elec_standoff_zoff])
					cylinder(d=elec_standoff_dia, h=elec_standoff_dz);
			      }
			 }
			 for(m=[0, 1]){
			      mirror([0, m, 0]){
				   hull(){
					translate([elec_standoff_xoff, elec_standoff_yoff, elec_standoff_zoff + elec_standoff_dz*0])
					     cylinder(d=elec_standoff_dia, h=0.1);
	  ...
	  color("green"){	// main optical beam
	       rotate([0, 90, 0])
		    translate([0, 0, -20]) cylinder(d=optical_beam_dia, h=40);		// main aperture for optical beam path
	  }
     }	  
     if (show_mount){
	  cs_position(){
	       cots_dc5v_camera_shutter(show=true, drill=false);
	  }
     }
}
